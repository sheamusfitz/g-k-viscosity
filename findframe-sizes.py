...
for lipid in ['dopc-293', 'dopc-303', 'dopc-313', 'dopc-323', 'dppc', 'dmpc', 'sopc', 'popc']:
  print(lipid)
  lipidfolder = 'boxsize/'+lipid
  print(lipidfolder)
  colnames = ['t','x','y','z']
  ...
  print('lipid: '+lipid+': xy-dimension: '+str(targetx))
  ...
  #   for i in range(5):
  #     ri = '/r'+str(i+1)
  #     grompp = 'gmx grompp -f ' + nvtfolder + '/step8_nvt.mdp -t ' + lipidfolder+ri + '/step7_boxsize.trr -time ' + str(bestarr[i]) + ' -o ' + nvtfolder+ri + '/step8_nvt.tpr -c ' + lipidfolder+ri + '/step7_boxsize.gro -n ' + nvtfolder + '/index.ndx -p ' + nvtfolder + '/topol.top'
  #     f.write(grompp+'\n')
  #   # print(grompp,'\n')
