...
skipping = 10000000  # 97000
# nrows = 10000

print('pandas importing')
bigpressure = pd.read_csv(xvgname, skiprows=33+skipping, header=None, delim_whitespace=True, chunksize=50000)
print('pandas got it')
...
stress = sp.zeros_like(bigpressure.xy)
for i in range(len(bigpressure)):
    stress[i] = sp.mean([bigpressure.xy[i], bigpressure.yx[i]])
# NOTE: this shifts the "stress" array down so that the mean is zero
# print(sp.mean(stress))
stress -= sp.mean(stress)
stressmean = [
    sp.mean([stress[sp.maximum(0, i - 25) : i + 1]]) for i in range(len(stress) - 1)
]
stressuncertainty = [
    sp.std([stress[: i + 1]]) / sp.sqrt(i + 1) for i in range(len(stress) - 1)
]
...
def autocor(arr, tau):
    ...
    return (sp.array([sp.mean(aaa), sp.stats.sem(aaa)]))
...
print(autocor(stress,0))
print(stress_autocor[0])
...
integrated = sp.array(
    [sp.sum(stress_autocor[: i + 1, 0]) for i in range(len(stress_autocor))]
)

print('integrated')

unclist = sp.array(
    [sp.sqrt(sp.sum(stress_autocor[:i, 1] ** 2))
...
def plotter():
    ...
    # plt.plot(xarr, integrated * viscosityfactor, '-', c='C0', alpha = 0.2,
    #   label='viscosity')
    ...
    plt.fill_between(
        xarr, visco_rolling + visco_sem, visco_rolling - visco_sem, color="C1",
        alpha=1,
    )
    ...
    plt.ylabel(r"Viscosity $(Pa\cdot m\cdot s)$")
    plt.xlabel(r"$\Delta\\tau$ (sec)")
    plt.xscale("log")
