...
box = pd.read_csv('box.xvg', header=None, skiprows=29, usecols=[1,2,3,4], names=colnames, sep='\t')
...
with open(lipidpath+'/grompp.txt', 'a') as f:
  grompp = 'gmx grompp -f ' + lipidpath + '/step8_nvt.mdp -t ' + runpath + '/step7_boxsize.trr -time ' + str(besttime) + ' -o ' + runpath + '/step8_nvt.tpr -c ' + runpath + '/step7_boxsize.gro -n ' + runpath + '/index.ndx -p ' + runpath + '/topol.top'
  f.write(grompp+'\n')
  # print(grompp,'\n')
